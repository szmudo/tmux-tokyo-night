...
function generate_left_side_string() {
    ...
    echo "#[fg=${PALLETE[fg_gutter]},bold]#{?client_prefix,#[bg=${PALLETE[yellow]}],#[bg=${PALLETE[green]}]} ${session_icon} #{?#{m|r:^#{WORKSPACE_GIT}/,#{session_path}},#{s|^#{WORKSPACE_GIT}/||:#{session_path}},#{session_name}} ${separator_end}"
}
